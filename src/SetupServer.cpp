...
void curl_send(SetupServer *s)
{
  ...
  if(curl) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "http://127.0.0.1:40001/post_key");
...
int main(void)
{
    s = new SetupServer(2, 3);
    curl_send(s);

    struct MHD_Daemon *daemon;

    daemon = MHD_start_daemon(MHD_USE_SELECT_INTERNALLY, 40000,
